...
void cut_mesh(const indexed_triangle_set& mesh, float z, indexed_triangle_set* upper, indexed_triangle_set* lower, bool triangulate_caps, std::vector<int>* upper_src_faces, std::vector<int>* lower_src_faces)
{
    assert(upper || lower);
    ...
    if (upper) {
        ...
        if (upper_src_faces) {
            upper_src_faces->clear();
            upper_src_faces->reserve(mesh.indices.size());
        }
    }
    ...
    if (lower) {
        ...
        if (lower_src_faces) {
            lower_src_faces->clear();
            lower_src_faces->reserve(mesh.indices.size());
        }
    }
    ...
    for (int facet_idx = 0; facet_idx < int(mesh.indices.size()); ++ facet_idx) {
        ...
        if (min_z > z || (is_equal(min_z , z) && max_z > z)) {
            ...
            auto new_face = [&facet_idx, upper, lower, upper_src_faces, lower_src_faces](indexed_triangle_set *its, int i, int j, int k) {
                if (its != nullptr && i != j && i != k && j != k) {
                    its->indices.emplace_back(i, j, k);
                    std::vector<int>* sf = (its == upper) ? upper_src_faces : lower_src_faces;
                    if (sf) sf->emplace_back(facet_idx);
                }
            };
